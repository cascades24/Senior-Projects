...
% Created: Jamison McGinley 10/7/19 based upon posModel3D by Anastasia Muszynski 
% Modified:  10/8/19
...
%% Running and plotting test cases
vec = {A1 A2 A3 Bee};
names = {'A1' 'A2' 'A3' 'Bee'};
range = linspace(-10,10,50);
for i = 1:length(vec)
   for j = 1:length(range)
        vec = {A1 A2 A3 Bee};
        vec{i} = vec{i}.*range(j);
        err(j,:)= test3D(vec{1}, vec{2}, vec{3}, vec{4}, f);
   end
        figure()
        subplot(2,2,1);
        plot(range, err(:,1));
...
function [err] = test3D(A1, A2, A3, Bee, f)
